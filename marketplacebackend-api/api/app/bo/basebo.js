...
class baseModelbo {
    request = null;
    response = null;
    constructor(baseModelDao, primaryKey, db) {
        ...
        if (params.meta_key && params.meta_key.length >= 3) {
            ...
            if (fieldsSearchMetas && fieldsSearchMetas.length) {
                ...
                fieldsSearchMetas.forEach(field_name => {
                    subConditions.push(Sequelize.where(Sequelize.fn("concat", Sequelize.col(field_name)), {ilike: "%" + params.meta_key.toUpperCase() + "%"}));
